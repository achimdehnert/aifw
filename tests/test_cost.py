...
class TestEstimateCostStandalone:
    ...
    def test_should_estimate_with_fallback_rates(self):
        ...
        with patch("aifw.cost._litellm") as mock_ll:
            mock_ll.return_value.cost_per_token.side_effect = Exception("not found")
    ...
    def test_should_use_litellm_when_available(self):
        with patch("aifw.cost._litellm") as mock_ll:
            mock_ll.return_value.cost_per_token.return_value = (0.001, 0.002)
            cost = estimate_cost(model="gpt-4o", input_tokens=1000, output_tokens=500)
            assert cost == Decimal("0.003")
            mock_ll.return_value.cost_per_token.assert_called_once_with(
                model="gpt-4o",
                prompt_tokens=1000,
...
class TestEstimateCostFromResult:
    ...
    def test_should_estimate_from_result(self):
        ...
        with patch("aifw.cost._litellm") as mock_ll:
            mock_ll.return_value.cost_per_token.return_value = (0.00015, 0.0003)
